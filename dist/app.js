...
class ToDoList {
        ...
        for (let i = 0; i < this.tasks.length; i++) {
            if (this.currentFilter === "all" ||
                ...
                checkboxElement.addEventListener("change", (event) => {
                    const isTaskCompleted = event.target.checked;
    ...
    saveTasksToLocalStorage() {
        const tasksJSON = JSON.stringify(this.tasks.map((task) => ({
    ...
    getTasksFromLocalStorage() {
        ...
        if (tasksJSON) {
            try {
                ...
                return tasksArray.map((task) => new TaskItem(task.taskName, task.taskPriority, task.isCompleted));
            }
            catch (e) {
